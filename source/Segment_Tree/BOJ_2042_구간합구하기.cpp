...
void update(ll idx, ll left, ll right, ll target, ll value){
  ...
  // 값이 변경되었으므로 재귀 빠져나오면서 구간합 정보들 또한 업데이트해줘야 함
  ll mid = (left+right)/2;
...
ll query(ll idx, ll left, ll right, ll target_L, ll target_R){
  ...
  if(target_R < left || right < target_L) return 0;
  
  // 현재 구간이 찾는 구간보다 작거나 같은 범위인 경우
  // 현재 구간합의 정보 챙겨가기
  if(target_L <= left && right <= target_R) return tree[idx];

  // 구간들의 합 종합해서 최종적으로 원하는 구간의 전체 합 구성하기
  ll mid = (left+right)/2;
...
/*
Segment Tree를 이용해서 구간합 구해주기
가장 기본적인 형태의 세그트리
tree 배열은 (인덱스, 좌, 우)의 정보를 바탕으로 원하는 위치 찾아감 + 분할정복 기법에 대한 이해

tree 배열 선언 시 개수에 대한 이해 
: 현재 가지고 있는 정보의 양이 이분트리 구조의 leaf 노드들로 와야하므로
N보다 큰 최소 2의 거듭제곱의 2배만큼의 배열 선언해주면 됨
대부분 N크기가 10^6이므로 이보다 큰 최소 2의 거듭제곱인 2^20의 2배만큼 선언
*/